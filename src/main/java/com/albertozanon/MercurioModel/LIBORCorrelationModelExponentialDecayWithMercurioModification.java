...
public class LIBORCorrelationModelExponentialDecayWithMercurioModification extends LIBORCorrelationModel {
	 ...
	 */
	public LIBORCorrelationModelExponentialDecayWithMercurioModification(final TimeDiscretization timeDiscretization, final TimeDiscretization liborPeriodDiscretization, final int numberOfFactors, final double a, final boolean isCalibrateable) {
	...
	public LIBORCorrelationModelExponentialDecayWithMercurioModification(final TimeDiscretization timeDiscretization, final TimeDiscretization liborPeriodDiscretization, final int numberOfFactors, final double a) {
		super(timeDiscretization, liborPeriodDiscretization);
	...
	@Override
	public LIBORCorrelationModelExponentialDecayWithMercurioModification getCloneWithModifiedParameter(final RandomVariable[] parameter) {
		if(!isCalibrateable) {
			return this;
		}

		return new LIBORCorrelationModelExponentialDecayWithMercurioModification(getTimeDiscretization(), getLiborPeriodDiscretization(), numberOfFactors, parameter[0].doubleValue());
	}

	@Override
	public Object clone() {
		return new LIBORCorrelationModelExponentialDecayWithMercurioModification(getTimeDiscretization(), getLiborPeriodDiscretization(), numberOfFactors, a, isCalibrateable);
	}
...
//				correlationMatrix[row][col] = Math.exp(-a * Math.abs(getLiborPeriodDiscretization().getTime(row)-getLiborPeriodDiscretization().getTime(col)));
				correlationMatrix[row][col] = Math.exp(-a * Math.abs(getLiborPeriodDiscretization().getTime(row+1)-getLiborPeriodDiscretization().getTime(col+1)));
	...
	public LIBORCorrelationModel getCloneWithModifiedData(final Map<String, Object> dataModified) {
		...
		final LIBORCorrelationModel newModel = new LIBORCorrelationModelExponentialDecayWithMercurioModification(timeDiscretization, liborPeriodDiscretization, numberOfFactors, a, isCalibrateable);
		return newModel;
	}
}
