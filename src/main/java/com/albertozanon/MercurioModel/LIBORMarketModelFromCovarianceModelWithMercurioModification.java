...
public class LIBORMarketModelFromCovarianceModelWithMercurioModification extends AbstractProcessModel implements LIBORMarketModel, Serializable {
	...
	public static LIBORMarketModelFromCovarianceModelWithMercurioModification of(
		...
		final LIBORMarketModelFromCovarianceModelWithMercurioModification model = new LIBORMarketModelFromCovarianceModelWithMercurioModification(liborPeriodDiscretization, analyticModel, forwardRateCurve, discountCurve, randomVariableFactory, covarianceModel, properties);
...
//		if(tenorPeriodStartTime < getTime(timeIndex)) {
		if(tenorPeriodEndTime < process.getTime(timeIndex)) {

			// Fixed at Long LIBOR period Start.
//------->MERCURIO		
//			timeIndex  = Math.min(timeIndex, getTimeIndex(tenorPeriodStartTime));
			timeIndex  = Math.min(timeIndex, process.getTimeIndex(tenorPeriodEndTime));
			if(timeIndex < 0) {
...
//				int evaluationTimeIndexForRealizations = Math.min(sumTimeIndex, getTimeIndex(getLiborPeriod(liborIndexForRealization)));
				int evaluationTimeIndexForRealizations = Math.min(sumTimeIndex, process.getTimeIndex(getLiborPeriod(liborIndexForRealization+1)));
	...
	//----->MERCURIO				if(getLiborPeriod(componentIndex1) > getTime(timeIndex)) {
							if(getLiborPeriod(componentIndex1+1) > simulationTimeDiscretization.getTime(timeIndex)) {
	...
	public Object clone() {
		try {
			...
			return LIBORMarketModelFromCovarianceModelWithMercurioModification.of(getLiborPeriodDiscretization(), getAnalyticModel(), getForwardRateCurve(), getDiscountCurve(), randomVariableFactory, covarianceModel, null, properties);
